    ...
    public partial class RaporlarPage : Page
    {
        ...
        private void LoadStats()
        {
            try
            {
                ...
                // Toplam kitap
                var cmd = new SqlCommand("SELECT ISNULL(SUM(MevcutAdet), 0) FROM Kitaplar", conn);
                txtToplamKitap.Text = cmd.ExecuteScalar()?.ToString() ?? "0";
                
                // Toplam üye
                cmd = new SqlCommand("SELECT COUNT(*) FROM Kullanicilar WHERE Rol = 'Uye'", conn);
                txtToplamUye.Text = cmd.ExecuteScalar()?.ToString() ?? "0";
                
                // Aktif ödünç
                cmd = new SqlCommand("SELECT COUNT(*) FROM OduncIslemleri WHERE IadeTarihi IS NULL", conn);
                txtAktifOdunc.Text = cmd.ExecuteScalar()?.ToString() ?? "0";
                
                // Geciken
                cmd = new SqlCommand("SELECT COUNT(*) FROM OduncIslemleri WHERE IadeTarihi IS NULL AND BeklenenIadeTarihi < GETDATE()", conn);
                txtGeciken.Text = cmd.ExecuteScalar()?.ToString() ?? "0";
                
                // Bu ay ödünç verilen
                cmd = new SqlCommand("SELECT COUNT(*) FROM OduncIslemleri WHERE MONTH(OduncTarihi) = MONTH(GETDATE()) AND YEAR(OduncTarihi) = YEAR(GETDATE())", conn);
                txtBuAyOdunc.Text = cmd.ExecuteScalar()?.ToString() ?? "0";
                
                // Bu ay iade alınan
                cmd = new SqlCommand("SELECT COUNT(*) FROM OduncIslemleri WHERE MONTH(IadeTarihi) = MONTH(GETDATE()) AND YEAR(IadeTarihi) = YEAR(GETDATE())", conn);
